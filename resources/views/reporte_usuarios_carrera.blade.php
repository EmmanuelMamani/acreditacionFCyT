...
<div class="flex justify-center">
    <div class="w-full lg:w-4/6 mt-10 grid grid-cols-10">
        <div class="col-span-6 md:col-span-8 lg:col-span-8">
            <h3 class="cursor-pointer text-md md:text-3xl justify-self-start">Usuarios</h3>
        </div>
...
<div class="overflow-x-auto">
    <table class=" w-full lg:w-4/6 mt-5 border-collapse table-auto" id='tabla'>
            <thead class="border-2 border-b-black  border-x-white border-t-white">
                <tr>
                    <th class="md:text-2xl">#</th>
                    <th class="md:text-2xl">Nombre</th>
                    <th class="md:text-2xl">Email</th>
                    <th class="md:text-2xl">Rol</th>
                    <th class="md:text-2xl">Accion</th>
                </tr>
            </thead>
            <tbody>
                ...
                <tr class="border border-y-stone-400 border-x-white">
                    ...
                    <th class="font-thin lg:text-xl">{{$usuario->name}}</th>
                    <th class="font-thin lg:text-xl">{{$usuario->email}}</th>
                    <th class="font-thin lg:text-xl">{{$usuario->rol_user->last()->rol->name}}</th>
                    <th>
    ...
    <dialog id="modal" class="w-5/6 sm:w-2/3 md:w-1/3 rounded-lg px-3">
        <div>
            <form action="{{route('registro_usuario')}}" method="post">
                ...
                <input type="password" name="password" class="bg-zinc-200 rounded-lg w-full p-2" value="{{old('password')}}"><br>
                @if ($errors->has('password'))
                <span class="text-red-700">{{ $errors->first('password') }}</span>
                @endif <br>
                <label class="font-thin">Confirmar contraseña</label><br>
                <input type="password" name="confirmacion" class="bg-zinc-200 rounded-lg w-full p-2" value="{{old('confirmacion')}}"><br>
